...
Actions.prototype.init = function () {
	...
	this.put(
		"smartFit",
		new Action(
			mxResources.get("fitWindow") + " / " + mxResources.get("resetView"),
			function () {
				...
				if (
					Math.abs(scale - graph.view.scale) < 0.00001 &&
	...
	this.addAction("pageSetup...", function () {
		ui.showDialog(new PageSetupDialog(ui).container, 320, 240, true, true);
	}).isEnabled = isGraphEnabled;
	this.addAction(
		"print...",
		function () {
	...
	this.addAction("swap", function () {
		...
		if (
			cells.length == 2 &&
			model.isVertex(cells[0]) &&
			model.isVertex(cells[1]) &&
	...
	this.put(
		"mergeCells",
		new Action(mxResources.get("merge"), function () {
	...
	this.put(
		"unmergeCells",
		new Action(mxResources.get("unmerge"), function () {
			var ss = ui.getSelectionState();
	...
	this.put(
		...
		new Action(
			...
			null,
			null,
			mxClient.IS_SF ? null : Editor.ctrlKey + "+R"
		)
	);
	this.put(
		"selectConnections",
		new Action(mxResources.get("selectEdges"), function (evt) {
			var cell = graph.getSelectionCell();
		...
		function () {
			if (!graph.isSelectionEmpty()) {
				...
				try {
					...
					var value =
						mxUtils.getValue(style, mxConstants.STYLE_EDITABLE, 1) == 1 ? 0 : 1;
	...
	function applyClipPath(cell, clipPath, width, height, graph) {
		...
		try {
			...
			if (geo != null && width && height) {
				//Comparing the ratio mostly will fail since it's float
	...
	this.addAction("image...", function () {
		if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
			...
			ui.showImageDialog(
				title,
				value,
				function (newValue, w, h, clipPath, cW, cH) {
	...
	this.addAction("crop...", function () {
		...
		if (
			...
			var value = style[mxConstants.STYLE_IMAGE],
			...
			var dlg = new CropImageDialog(
				ui,
				value,
				style[mxConstants.STYLE_CLIP_PATH],
				function (clipPath, width, height) {
	...
	action = this.addAction(
		...
		mxUtils.bind(this, function () {
			if (this.layersWindow == null) {
				...
				this.layersWindow.window.addListener(
					"show",
					mxUtils.bind(this, function () {
	...
	action = this.addAction(
		...
		mxUtils.bind(this, function () {
			if (this.outlineWindow == null) {
				...
				this.outlineWindow.window.addListener(
					"show",
					mxUtils.bind(this, function () {
		...
		mxUtils.bind(this, function () {
			if (this.animationWindow == null) {
				...
				this.animationWindow = new AnimationWindow(
					ui,
					(document.body.offsetWidth - 380) / 2,
					120,
					330,
					240
				);
				if (this.animationWindow.window != undefined) {
					this.animationWindow.window.setVisible(true);
					this.animationWindow.window.setScrollable(true);
					//this.animationWindow.window.setResizable(false);
				}
				...
				document.getElementById("presentationSidebar").style.visibility ===
				"hidden"
			) {
				document.getElementById("presentationSidebar").style.visibility =
					"visible";
				document.getElementById("getPresentation").style.visibility = "visible";
				// document.getElementById("animationType").style.visibility = "visible";
			} else {
				this.animationWindow.hideAnimationWindow()
			}
		}),
		null,
		null,
		Editor.ctrlKey + "+Shift+^"
	);
	...
	action.setSelectedCallback(
		mxUtils.bind(this, function () {
			return (
				...
				this.animationWindow.window != null &&
